...
export const signinService = async (
    ...
    if (
        ...
        throw apiError.badRequest(
            `Unsupported code_challenge_method: "${code_challenge_method}". ProtoAuth requires "S256".`,
        );
    }
...
export const signupService = async (
    ...
    if (!newUser) {
        throw apiError.internal("Failed to register user account");
    }
...
export const signoutService = async (id_token: string) => {
    if (!id_token) throw apiError.badRequest("Missing id_token");

    let decoded: { sub: string };
    try {
        decoded = jwt.verify(id_token, PUBLIC_KEY, {
            algorithms: ["RS256"],
        }) as { sub: string };
    } catch {
        throw apiError.unauthorized("Invalid or expired id_token");
    }
...
export const tokenService = async (body: TokenType) => {
    const {
        grant_type,
        client_id,
        client_secret,
    ...
    if (grant_type === "authorization_code") {
        ...
        // Verify client secret if configured on server (Confidential Client)
        if (client.clientSecret && client.clientSecret !== client_secret) {
            throw apiError.unauthorized("Invalid client_secret credentials");
        }
        ...
        return await db.transaction(async (tx) => {
            ...
            if (!stored) {
                throw apiError.badRequest(
                    "Invalid or spent authorization code",
                );
            }

            // 2. Burn code immediately to prevent reuse / replay attacks
            await tx
                .delete(authCodesTable)
                .where(eq(authCodesTable.code, code));

            // 3. Validate code scope and expiry
            if (
            ...
            if (stored.codeChallenge) {
                ...
                const bufA = Buffer.from(calculatedChallenge);
                const bufB = Buffer.from(stored.codeChallenge);
                ...
                // Enforce PKCE for public clients without secrets
                throw apiError.badRequest(
