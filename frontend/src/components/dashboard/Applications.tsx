...
import OneTimeSecretComponent from './OneTimeSecretComponent';
...
const Applications = () => {
	...
	const [createdSecretData, setCreatedSecretData] =
		useState<NewClientData | null>(null);
	...
	const onSubmit = async (data: registerType) => {
		...
		const toastId = toast.loading('Registering your application...');

		try {
			const res = await api.post('/api/admin/application', formattedData);

			// Handles both { data: { clientId, clientSecret, name } } and { clientId, clientSecret, name }
			...
			const clientId = appData?.clientId || appData?.client_id;
			const clientSecret = appData?.clientSecret || appData?.client_secret;

			if (clientId && clientSecret) {
				setCreatedSecretData({
					clientId,
					clientSecret,
					name: appData.name || data.websiteName,
				});
			} else {
				navigate('/dashboard/clients');
			}
		} catch (error) {
			let message = 'Failed to register website';
	...
	return (
		<section className="relative p-4 md:p-6 space-y-6 mx-auto">
			{createdSecretData && (
				<OneTimeSecretComponent
									...
									{fields.map((field, index) => (
										...
										>
											<div className="flex gap-2">
												<input
													{...register(`redirectUris.${index}.value` as const, {
											...
											{errors.redirectUris?.[index]?.value && (
												<p className="text-[10px] text-red-500 font-medium">
													{errors.redirectUris[index]?.value?.message}
												</p>
											)}
										</div>
									))}
					...
					<Card className="bg-blue-50/50 dark:bg-blue-500/5 border-blue-100 dark:border-blue-500/10">
						<CardContent className="p-4 space-y-3">
							<div className="flex items-center gap-2 text-blue-600 dark:text-blue-400">
								<Icons.Alert size={20} />
								<span className="text-sm font-bold">Security Note</span>
							</div>
							<p className="text-[12px] text-blue-700 dark:text-blue-300 leading-relaxed">
								Only requests coming from these registered URIs will be
								accepted. For local development, you may use{' '}
					...
					<Card className="border-neutral-200 dark:border-neutral-800">
						<CardContent className="p-4 space-y-3">
							...
							<p className="text-[11px] text-neutral-500 leading-relaxed">
								After registration, you will receive a <b>Client ID</b> and{' '}
								<b>Client Secret</b>. Keep the secret safe—it cannot be
								recovered if lost.
							</p>
						</CardContent>
					</Card>
